...
use super::{
    ...
    request::*,
    Error, CONFIG_SPACE_SIZE, QUEUE_SIZE, QUEUE_SIZES, SECTOR_SHIFT, SECTOR_SIZE,
};
...
impl Block {
    ...
    pub fn new(
        ...
        let ring = io_uring::IoUring::new(QUEUE_SIZE as u32).unwrap();
    ...
    pub(crate) fn event_idx(&mut self) -> bool {
    ...
    pub(crate) fn process_queue(&mut self, queue_index: usize) -> bool {
                        ...
                        Ok(l) => {
                                ...
                                && request.request_type != RequestType::Out
                            {
                                self.mem
                                    ...
                                    .unwrap();
                                len = l;
                                used_desc_heads[used_count] = (head.index, len);
                                used_count += 1;
                            } else {
                                self.in_flight += 1;
                            }
        ...
        for &(desc_index, len) in &used_desc_heads[..used_count] {
            if let Some(used_idx) = self.queues[queue_index].add_used(&self.mem, desc_index, len) {
...
impl VirtioDevice for Block {
    ...
    fn set_device_activated(&mut self, device_activated: bool) {
        ...
        println!("EVENT_IDX is set? {}", self.event_idx());
    }
...
mod tests {
    ...
    fn test_invalid_request() {
        ...
        mem.write_obj_at_addr::<u32>(VIRTIO_BLK_T_IN, request_type_addr)
            .unwrap();
    ...
    fn test_request_execute_failures() {
...

        {
            ...
            mem.write_obj_at_addr::<RequestHeader>(request_header, request_type_addr)
                .unwrap();
            ...
            assert_eq!(
                mem.read_obj_from_addr::<u32>(status_addr).unwrap(),
                VIRTIO_BLK_S_IOERR
            );
        }
    ...
    fn test_read_write() {
        ...
        // Write.
        {
            mem.write_obj_at_addr::<u32>(VIRTIO_BLK_T_OUT, request_type_addr)
                .unwrap();
            // Make data read only, 8 bytes in len, and set the actual value to be written.
            vq.dtable[1].flags.set(VIRTQ_DESC_F_NEXT);
            vq.dtable[1].len.set(8);
            mem.write_obj_at_addr::<u64>(123_456_789, data_addr)
                .unwrap();
        ...
        // Read.
        {
            ...
            assert_eq!(
                mem.read_obj_from_addr::<u64>(data_addr).unwrap(),
                123_456_789
            );
        }
    }
    ...
    fn test_flush() {
        ...
        // Flush completes successfully without a data descriptor.
        {
            ...
            mem.write_obj_at_addr::<u32>(VIRTIO_BLK_T_FLUSH, request_type_addr)
                .unwrap();
    ...
    fn test_get_device_id() {
        ...
        // Test that the driver receives the correct device id.
        {
            ...
            mem.write_obj_at_addr::<u32>(VIRTIO_BLK_T_GET_ID, request_type_addr)
                .unwrap();
            ...
            assert!(mem.read_slice_at_addr(&mut buf, data_addr).is_ok());
